...
const AddInstitution: React.FC = () => {
  ...
  const [coursesAndFees, setCoursesAndFees] = useState([{ course: "", duration: "", fees: "" }]);
  ...
  const handleCourseChange = (index: number, field: "course" | "fees", value: string) => {
  ...
  const handleAddCourse = () => setCoursesAndFees([...coursesAndFees, { course: "", duration: "", fees: "" }]);
  ...
  return (
    <div className="max-w-4xl mx-auto p-16 bg-white rounded shadow">
      ...
      <form onSubmit={handleAddInstitution} className="space-y-4">
        ...
        <select value={entranceExam} onChange={(e) => setEntranceExam(e.target.value)} className="w-full border rounded p-2">
          <option value="">Select Entrance Exam</option>
          {entranceExamOptions.map((exam, index) => <option key={index} value={exam}>{exam}</option>)}
        </select>
        ...
        {coursesAndFees.map((item, index) => (
          <div key={index} className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <select value={item.course} onChange={(e) => handleCourseChange(index, "course", e.target.value)} className="border rounded p-2">
              <option value="">Select Course</option>
              {courseOptions.map((course, idx) => {
                const isSelected = coursesAndFees.some((c, i) => c.course === course.name && i !== index);
                return <option key={idx} value={course.name} disabled={isSelected}>{course.name}</option>;
              })}
            </select>
            <input type="text" value={item.duration || ""} placeholder="Duration" readOnly className="border rounded p-2 bg-gray-100" />
            <input type="text" placeholder="Fees" value={item.fees} onChange={(e) => handleCourseChange(index, "fees", e.target.value)} className="border rounded p-2" />
          </div>
        ))}
        <button type="button" onClick={handleAddCourse} className="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700">+ Add Course</button>
