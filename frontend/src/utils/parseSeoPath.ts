...
export default function parseSeoPath(input: string, dynamicCourses: string[]): ParsedFilters {
  ...
  filters.entranceExam = fuzzyMatch(input, entranceExams);

  // Match course using dynamic list first
  const matchedSlug = fuzzyMatch(input, dynamicCourses);
  const matchedCourse = staticCourses.find((c) => c.slug === matchedSlug);
  if (matchedCourse) {
